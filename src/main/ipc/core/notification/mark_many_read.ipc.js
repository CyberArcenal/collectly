...
module.exports = async (params, queryRunner) => {
  ...
  if (mode === "online") {
    ...
    const response = await onlineClient.post('/api/v1/notifications/mark-many-read/', { ids });
    if (!response.ok) {
    ...
    return {
      status: true,
      message: "Notifications marked as read on server",
      data: extractData(serverResult), // { updatedCount }
    };
  } else {
