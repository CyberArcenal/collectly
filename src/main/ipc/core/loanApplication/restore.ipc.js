...
module.exports = async (params, queryRunner) => {
  ...
  if (mode === "online") {
    ...
    const response = await onlineClient.post(`/api/v1/loan_applications/${id}/restore/`, { user });
    if (!response.ok) {
    ...
    return {
      status: true,
      message: "Loan application restored on server",
    ...
    return {
      status: true,
      message: "Loan application restored locally",
      data: result,
    };
  }
};