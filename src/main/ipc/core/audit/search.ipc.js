// src/main/ipc/core/audit/search.ipc.js
//@ts-check
...
module.exports = async (params) => {
  ...
  if (!params.searchTerm || params.searchTerm.trim() === "") {
    return { status: false, message: "Search term is required", data: null };
  }
  ...
  if (mode === "online") {
    ...
    const response = await onlineClient.get("/api/v1/audit/search/", {
      params: query,
    });
    if (!response.ok) {
    ...
    if (startDate && endDate) {
      qb = qb.andWhere("log.timestamp BETWEEN :start AND :end", {
