...
const RecordPeriodPaymentModal: React.FC<RecordPeriodPaymentModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message.includes("already paid")) {
        ...
        onSuccess(); // Refresh to show correct status
        onClose();
      } else {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Record Payment - ${debtor.borrowerName}`}
      size="md"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          <p>
            <strong style={{ color: "var(--text-primary)" }}>Debtor:</strong>{" "}
          ...
          <p>
            ...
            <span style={{ color: "var(--debt-high)" }}>
          ...
          >
            This will record payments for {debtor.debts.length} debt(s) for this
            period.
          </div>
        </div>
