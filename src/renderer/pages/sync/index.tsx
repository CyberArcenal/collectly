...
const SyncPage: React.FC = () => {
  ...
  const statusConfig = useMemo(() => {
    if (isSyncing) return { label: "Syncing", color: "text-yellow-500", bg: "bg-yellow-500/10", dot: "animate-pulse" };
    if (error) return { label: "Error", color: "text-red-500", bg: "bg-red-500/10", dot: "" };
    if (pendingChangesCount > 0) return { label: "Changes Pending", color: "text-blue-500", bg: "bg-blue-500/10", dot: "" };
    return { label: "Online & Synced", color: "text-green-500", bg: "bg-green-500/10", dot: "" };
  }, [isSyncing, error, pendingChangesCount]);

  return (
    // 🔥 Inalis ang explicit na background para gamitin ang parent (card) background
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--border-color)] bg-[var(--card-bg)]/90 backdrop-blur-sm rounded-t-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-bold text-[var(--text-primary)] tracking-tight">Data Sync</h1>
            <p className="text-xs text-[var(--text-secondary)]">Synchronize local data with the server</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {/* Status Chip */}
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${statusConfig.bg} border-[var(--border-color)]`}>
            <span className={`w-2 h-2 rounded-full ${statusConfig.dot} ${statusConfig.color.replace('text', 'bg')}`} />
          ...
          <div className="hidden md:flex items-center gap-2 text-xs text-[var(--text-tertiary)] bg-[var(--card-secondary-bg)] px-3 py-1.5 rounded-full border border-[var(--border-color)]">
            ...
            <span>Last sync: <span className="text-[var(--text-secondary)] font-medium">{getRelativeTime(lastSyncAt)}</span></span>
          </div>
          {/* Refresh Button */}
          <button
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5">
        ...
        {error && (
          <div className="flex items-center justify-between bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 text-red-500 text-sm">
            ...
            <button onClick={handleRefresh} className="text-xs font-medium hover:underline">Retry</button>
          </div>
        )}
        ...
        <div className="flex items-center justify-between text-xs text-[var(--text-tertiary)] pt-2 border-t border-[var(--border-color)] mt-2">
          <div className="flex items-center gap-4">
            ...
            <span className="flex items-center gap-1.5">
              {isSyncing ? (
                <><Loader2 className="w-3 h-3 animate-spin text-yellow-500" /> Syncing...</>
              ) : (
