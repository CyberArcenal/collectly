...
} from "../../../../api/core/payment_transaction";
import penaltiesAPI, {
  type PenaltyTransaction,
} from "../../../../api/core/pernalty_transaction";
...
const ViewDebtModal: React.FC<ViewDebtModalProps> = ({
  isOpen,
  debt: initialDebt,
  onClose,
}) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "overdue":
        return "bg-[var(--status-overdue-bg)] text-[var(--status-overdue-text)]";
      case "paid":
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <span
                    className={`inline-block px-2 py-0.5 text-xs rounded-full mt-0.5 ${getStatusBadge(debt.status)}`}
                  >
                    {debt.status}
                  </span>
                </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-1 text-sm mt-1">
                  ...
                  <div className="col-span-2">
                    ...
                    <span className="ml-1 text-[var(--text-primary)]">
                      {debt.interestCalculationPeriod === "per_annum"
                        ? "Per Annum"
                        : debt.interestCalculationPeriod === "per_month"
                          ? "Per Month"
                          : "—"}
                    </span>
                  </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-1 text-sm mt-1">
                  ...
                  <div>
                    ...
                    <span className="ml-1 text-[var(--text-primary)]">
                      {debt.updatedAt ? formatDate(debt.updatedAt) : "—"}
                    </span>
                  </div>
                  {debt.lastInterestAccrualDate && (
                    <div className="col-span-2">
                      ...
                      <span className="ml-1 text-[var(--text-primary)]">
                        {formatDate(debt.lastInterestAccrualDate)}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="text-[var(--text-secondary)] border-b border-[var(--border-color)]">
                      <tr>
                        <th className="text-left py-1.5 px-2 text-[10px] uppercase tracking-wider">
                          Date
                        </th>
                        <th className="text-right py-1.5 px-2 text-[10px] uppercase tracking-wider">
                          Amount
                        </th>
                        <th className="text-left py-1.5 px-2 text-[10px] uppercase tracking-wider">
                          Reference
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {penalties.map((p) => (
                        ...
                        >
                          <td className="py-1.5 px-2">
                            {p.penaltyDate ? formatDate(p.penaltyDate) : "—"}
                          </td>
